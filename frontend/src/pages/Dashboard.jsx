...
export default function Dashboard() {
  ...
  const isHome = normalizedPath === '/dashboard' || normalizedPath === '/admin';
  const isAccounts = normalizedPath === '/dashboard/comptes' || normalizedPath === '/admin/comptes';
  const isHistory = normalizedPath === '/dashboard/historique' || normalizedPath === '/admin/historique';
  const isSettings = normalizedPath === '/dashboard/parametres' || normalizedPath === '/admin/parametres';
  const isReclamations = normalizedPath === '/dashboard/reclamations' || normalizedPath === '/admin/reclamations';
  const isTestimonials = normalizedPath === '/dashboard/temoignages' || normalizedPath === '/admin/temoignages';
  ...
  const pendingAccounts = useMemo(() => {
    const rows = accountsData?.accounts || [];
    return rows.filter((a) => String(a?.status || '').toLowerCase() === 'en attente');
  }, [accountsData]);
      ...
      <main className="relative z-10 flex-1 xl:ml-64">
        <div className="sticky top-0 z-10 border-b border-slate-200/90 bg-white px-4 py-2.5 md:px-6 md:py-2.5">
          ...
          <div className="mt-2 flex items-center gap-2 overflow-x-auto pb-1 xl:hidden">
            ...
            <NavLink to="/dashboard/temoignages" className={({ isActive }) => `whitespace-nowrap rounded-xl px-3 py-2 text-sm font-semibold ${isActive ? 'bg-cyan-500 text-white' : 'bg-slate-100 text-slate-600'}`}>Témoignages</NavLink>
